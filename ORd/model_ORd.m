function output=model_ORd11(t,X,flag_ode, parameter_inputs, gendertype, cell_type, par_SA)

voltageclamp = 0;
CL = parameter_inputs.CL;
stim1_amp = parameter_inputs.stim1_amp;
stim1_dur = parameter_inputs.stim1_dur;
IK1_scale= parameter_inputs.IK1_scale;
INa_scale=  parameter_inputs.INa_scale;
INaL_scale= parameter_inputs.INaL_scale;

mutation_flag_LQT8 = parameter_inputs.mutation_flag_LQT8;
mutation_change_GCaL = parameter_inputs.mutation_change_GCaL;
mutation_change_vCaL = parameter_inputs.mutation_change_vCaL;
ICaL_scale = parameter_inputs.ICaL_scale*(1+mutation_change_GCaL*mutation_flag_LQT8);

mutation_flag_LQT2 = parameter_inputs.mutation_flag_LQT2;
mutation_change_GKr = parameter_inputs.mutation_change_GKr;
% 1-mutation because loss of function
IKr_scale = parameter_inputs.IKr_scale*(1-mutation_change_GKr*mutation_flag_LQT2);

mutation_flag_LQT3 = parameter_inputs.mutation_flag_LQT3;
mutation_change_GNaL = parameter_inputs.mutation_change_GNaL;
INaL_scale = INaL_scale*(1+mutation_change_GNaL*mutation_flag_LQT3);

celltype= cell_type; %endo = 0, epi = 1, M = 2

INa_Multiplier = par_SA(1);   %'GNa' 
INaL_Multiplier = par_SA(2);  %'GNaL'
Ito_Multiplier = par_SA(3);   %'Gto'
ICaL_Multiplier = par_SA(4);  %'PCa'
IKr_Multiplier = par_SA(5);   %'GKr'
IKs_Multiplier = par_SA(6);   %'GKs'
IK1_Multiplier = par_SA(7);   %'GK1'
INaCa_Multiplier = par_SA(8); %'Gncx'
INaK_Multiplier = par_SA(9);  %'Pnak'
IKB_Multiplier = par_SA(10);  %'GKb'
INaB_Multiplier = par_SA(11); %'PNab'
ICaB_Multiplier = par_SA(12); %'PCab'
IpCa_Multiplier = par_SA(13); %'GpCa'
Jup_Multiplier = par_SA(14);  %'fSERCA'
Jrel_Multiplier = par_SA(15); %'fRyR'
Jleak_Multiplier = par_SA(16);%'fIleak'
...
INa= INa_Multiplier*INa_scale*GNa*(v-ENa)*m^3.0*((1.0-fINap)*h*j+fINap*hp*jp);
...
INaL=INaL_Multiplier*INaL_scale*GNaL*(v-ENa)*mL*((1.0-fINaLp)*hL+fINaLp*hLp);
...
if gendertype == 1
    if celltype == 1; scaleItos = 0.6;
    elseif celltype == 0; scaleItos = 1; end
    
elseif gendertype == 2
    if celltype == 1; scaleItos = 0.26;
    elseif celltype == 0; scaleItos = 0.64; end
...
Gto=0.02;
if celltype==1
    Gto=Gto*2.0; %Scale by 2 instead of 4
%elseif celltype==2
%    Gto=Gto*4.0;
end
fItop=(1.0/(1.0+KmCaMK/CaMKa));
Ito=Ito_Multiplier*Gto*(v-EK)*((1.0-fItop)*a*i+fItop*ap*ip);

%calculate ICaL, ICaNa, ICaK
dss=1.0/(1.0+exp((-(v+3.940+mutation_change_vCaL*mutation_flag_LQT8))/4.230));
...
zca=2.0;
PCa=0.0001*ICaL_scale*ICaL_Multiplier;
if celltype==1
    PCa=PCa*1.2*ICaL_scale;
%elseif celltype==2
%    PCa=PCa*2.5;
end
PCap=1.1*PCa;
...
% if gendertype == 1
%     if celltype == 1; GKr=0.046*1.09;
%     elseif celltype == 0; GKr=0.046*1; end
% 
% elseif gendertype == 2
%     if celltype == 1; GKr=0.046*0.875;
%     elseif celltype == 0; GKr=0.046*0.79; end
% 
% else 
%     GKr=0.046;
%     if celltype==1; GKr=GKr*1.3;
%     elseif celltype==2; GKr=GKr*0.8; end
   
%end

% if gendertype == 1
%     if celltype == 1; GKr=0.06;
%     elseif celltype == 0; GKr=0.046; end
% 
% elseif gendertype == 2
%     if celltype == 1; GKr=0.06*0.875;
%     elseif celltype == 0; GKr=0.046*0.79; end
% 
% else 
%     GKr=0.046;
...
elseif gendertype == 2
    if celltype == 1; GKr= 0.0744*0.8;
    elseif celltype == 0; GKr=0.04*0.8; end
...
IKr=IKr_Multiplier*IKr_scale*GKr*sqrt(ko/5.4)*xr*rkr*(v-EK);
...
%     GKs=GKs*1.4;
% end
%also scaling the txs1 with 1.04, SS equation
if gendertype == 1
    if celltype == 1
        xs1ss=1.0./(1.0+exp((-(v+11.60))/8.932)*1.04);
        txs1=817.3+1.0./(2.326e-4*exp((v+48.28)/17.80)*1.04+0.001292*exp((-(v+210.0))/230.0)*1.04);
        xs2ss=xs1ss;
        txs2=1.0./(0.01*exp((v-50.0)/20.0)*1.04+0.0193*exp((-(v+66.54))/31.0)*1.04);
        GKs = 0.0034*1.04;

    elseif celltype == 0
        xs1ss=1.0./(1.0+exp((-(v+11.60))/8.932)*1);
        txs1=817.3+1.0./(2.326e-4*exp((v+48.28)/17.80)*1+0.001292*exp((-(v+210.0))/230.0)*1);
        xs2ss=xs1ss;
        txs2=1.0./(0.01*exp((v-50.0)/20.0)*1+0.0193*exp((-(v+66.54))/31.0)*1);
        GKs = 0.0034*1;

    end
elseif gendertype == 2
    if celltype == 1
        xs1ss=1.0./(1.0+exp((-(v+11.60))/8.932)*0.87);
        txs1=817.3+1.0./(2.326e-4*exp((v+48.28)/17.80)*0.87+0.001292*exp((-(v+210.0))/230.0)*0.87);
        xs2ss=xs1ss;
        txs2=1.0./(0.01*exp((v-50.0)/20.0)*0.87+0.0193*exp((-(v+66.54))/31.0)*0.87);
        GKs = 0.0034*0.87;
        
    elseif celltype == 0
        xs1ss=1.0./(1.0+exp((-(v+11.60))/8.932)*0.83);
        txs1=817.3+1.0./(2.326e-4*exp((v+48.28)/17.80)*0.83+0.001292*exp((-(v+210.0))/230.0)*0.83);
        xs2ss=xs1ss;
        txs2=1.0./(0.01*exp((v-50.0)/20.0)*0.83+0.0193*exp((-(v+66.54))/31.0)*0.83);
        GKs = 0.0034*0.83;
...
dxs2=(xs2ss-xs2)/txs2;
KsCa=1.0+0.6/(1.0+(3.8e-5/cai)^1.4);
IKs=IKs_Multiplier*GKs*KsCa*xs1*xs2*(v-EKs);
...
if gendertype == 1
    if celltype == 1; GK1=0.1908*0.98;
    elseif celltype == 0; GK1=0.1908; end
    
elseif gendertype == 2
    if celltype == 1; GK1=0.1908*0.74;
    elseif celltype == 0; GK1=0.1908*0.86; end
...
IK1=IK1_Multiplier*IK1_scale*GK1*sqrt(ko)*rk1*xk1*(v-EK);
...
INaCa_i=INaCa_Multiplier*0.8*Gncx*allo*(zna*JncxNa+zca*JncxCa)*scaleNaCas;
...
JncxNa=3.0*(E4*k7-E1*k8)+E3*k4pp-E2*k3pp;
JncxCa=E2*k2-E1*k1;
INaCa_ss=INaCa_Multiplier*0.2*Gncx*allo*(zna*JncxNa+zca*JncxCa)*scaleNaCas;
...
%Fogli-Iseppe 2021 removed sex-differences in Pnak
% if celltype==1
%     %Pnak=Pnak*0.9;
%     Pnak=Pnak*0.94;  %Pei-Chi, Alex
% elseif celltype==2
%     Pnak=Pnak*0.7;
% end
%if gendertype == 1
    if celltype == 1; Pnak=30*0.94;
    elseif celltype == 0; Pnak=30*1; end
    
% elseif gendertype == 2
%     if celltype == 1; Pnak=30*0.7;
%     elseif celltype == 0; Pnak=30*0.79; end
% 
% else 
%     Pnak=30;
%     if celltype==1; Pnak=Pnak*0.9;
%     elseif celltype==2; Pnak=Pnak*0.7; end
% 
% end
INaK=INaK_Multiplier*Pnak*(zna*JnakNa+zk*JnakK);
...
IKb=IKB_Multiplier*GKb*xkb*(v-EK);

%calculate INab
PNab=3.75e-10;
INab=INaB_Multiplier*PNab*vffrt*(nai*exp(vfrt)-nao)/(exp(vfrt)-1.0);

%calculate ICab
PCab=2.5e-8;
ICab=ICaB_Multiplier*PCab*4.0*vffrt*(cai*exp(2.0*vfrt)-0.341*cao)/(exp(2.0*vfrt)-1.0);

%calculate IpCa
GpCa=0.0005;

if gendertype == 1
    if celltype == 1; GpCa=0.0005*0.88;
    elseif celltype == 0; GpCa=0.0005*1; end
    
elseif gendertype == 2
    if celltype == 1; GpCa=0.0005*1.6;
    elseif celltype == 0; GpCa=0.0005*1.6; end
     
end

IpCa=IpCa_Multiplier*GpCa*cai/(0.0005+cai);
...
% For Voltage Clamp:
if voltageclamp==0 
    v_clamp= v; %set i_voltageclamp to 0    
elseif voltageclamp==1    % train of square pulse:
    %if mod(t,CL) < CL-steplength
    if t<= steplength
...
i_voltageclamp=(v_clamp- v)/R_clamp;



%udtate the membrane voltage
dv=-(INa+INaL+Ito+ICaL+ICaNa+ICaK+IKr+IKs+IK1+INaCa_i+INaCa_ss+INaK+INab+IKb+IpCa+ICab+Istim - i_voltageclamp);
...
Jrel=Jrel_Multiplier*(1.0-fJrelp)*Jrelnp+fJrelp*Jrelp;

%calculate serca pump, ca uptake flux
Jupnp=0.004375*cai/(cai+0.00092);
Jupp=2.75*0.004375*cai/(cai+0.00092-0.00017);
% if celltype==1
%     %Jupnp=Jupnp*1.3;  %Original
%     %Jupp=Jupp*1.3;    %Original
%     Jupnp=Jupnp*1.42;  %Pei-Chi, Alex
%     Jupp=Jupp*1.42;    %Pei-Chi, Alex
% end
...
Jleak=Jleak_Multiplier*0.0039375*cansr/15.0;
Jup=Jup_Multiplier*(1.0-fJupp)*Jupnp+fJupp*Jupp-Jleak;
...
if gendertype == 1
    if celltype == 1; cmdnmax=0.05*1.07;
    elseif celltype == 0; cmdnmax=0.05*1; end
    
elseif gendertype == 2
    if celltype == 1; cmdnmax=0.05*1.41;
    elseif celltype == 0; cmdnmax=0.05*1.21; end
...
if flag_ode==1
    ...
    output=[INa INaL Ito ICaL IKr IKs IK1 INaCa_i INaCa_ss INaK  IKb INab ICab IpCa Jdiff JdiffNa JdiffK Jup Jleak Jtr Jrel CaMKa Istim ICaNa ICaK PhiCaL PhiCaNa PhiCaK];
end
